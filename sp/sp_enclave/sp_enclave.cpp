...
sgx_status_t ecall_do_attestation(ra_msg01_t *msg01,
                                  ra_msg4_t *msg4, attestation_status_t *att_status, attestation_error_t *att_error) {

    *att_status = NotTrusted;
    sgx_status_t status;

    if (!msg01 || !msg4 || !att_status || !att_error) {
        return SGX_ERROR_INVALID_PARAMETER;
    }

//    proc msg0
    status = private_proc_msg0(&secret, msg01->msg0_extended_epid_group_id, att_error);
...
//    [ocall]get  SigRL, spid, quote_type
    ...
    uint16_t quote_type;
    uint32_t sigrl_size;
    ocall_pre_get_sigrl(secret.client_gid, &sp_spid, &quote_type, &sigrl_size);

    uint8_t *sigrl = (uint8_t *) malloc(sigrl_size);
    ocall_get_sigrl(sigrl_size, sigrl);
