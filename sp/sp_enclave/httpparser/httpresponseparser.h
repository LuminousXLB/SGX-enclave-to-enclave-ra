                    ...
                    case HeaderValue:
                        if (input == '\r') {
                            ...
                            if (strcasecmp(h.name.c_str(), "Content-Length") == 0) {
                                contentSize = strtol(h.value.c_str(), nullptr, 10);
//                                contentSize = atoi(h.value.c_str());
                    ...
                    case ExpectingNewline_3: {
                        auto it = std::find_if(resp.headers.begin(), resp.headers.end(), checkIfConnection);

                        if (it != resp.headers.end()) {
                            resp.keepAlive = strcasecmp(it->value.c_str(), "Keep-Alive") == 0;
                        } else {
